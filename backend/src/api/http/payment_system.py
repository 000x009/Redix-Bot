...
from src.api.schema.payment import TopUpSchema, CryptoPayTopUpSchema
...
from src.services.cryptopay import CryptopayClientImpl
...
@router.post('/crypto-pay/top-up')
@inject
async def crypto_top_up(
    data: CryptoPayTopUpSchema,
    cryptopay_client: CryptopayClientImpl = Depends(Provide[Container.cryptopay_client]),
    transaction_service: TransactionService = Depends(Provide[Container.transaction_service]),
    user_data: WebAppInitData = Depends(user_provider),
) -> JSONResponse:
...
@router.post("/crypto-pay/webhook", response_class=JSONResponse)
@inject
async def receive_payment(
    request: Request,
    transaction_service: TransactionService = Depends(Provide[Container.transaction_service]),
    ...
    try:
        ...
        if payload.get('update_type') == 'invoice_paid':
            ...
            payment_id = uuid.UUID(invoice.get('payload'))
            if not payment_id:
                return JSONResponse(status_code=400, content={"error": "Missing payment ID"})
